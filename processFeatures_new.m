...
%%
clc, clear, close all
addpath(genpath('interdist'));
% Change the file name accordingly.
param.modelName = 'DenseNet121';
%1D or 2D corresponding to the traditional and proposed dictionary designs.
param.DicDesign = '2D';
inputData = strcat('features/features_new_', param.modelName, '.mat');
load(inputData)
objectFeatures = double(objectFeatures);
angles = gtd(:, 1);
meters = gtd(:, 2);
outName = strcat('competing_splits/interdist/', param.modelName);
if ~exist('competing_splits/interdist/', 'dir')
   mkdir('competing_splits/interdist/')
end    
